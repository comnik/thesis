...
Consider a three-way join $(\Join A B C)$, implemented as a nested
dataflow with a fixed order $(\Join (\Join A B) C)$.

For such a plan, Differential dataflow will by default maintain
arrangements of every intermediate result ($(\Join A B)$ in this
case), incurring additional memory overhead and some time spent
managing the shared trace.
...
@TODO cardinality estimation can not avoid disastrous plans, because
(a) we need correlated relations in practice, in order to model
functional dependencies as discrete mappings, (b) we don't have
meaningful statistics on external sources (e.g. parameter
collections), and (c) any overly optimistic estimation will
immediately cause errors proportional in the size of the relation. And
of course, the order might be highly data dependent.
